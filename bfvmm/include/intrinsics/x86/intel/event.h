...
#ifndef EVENT_INTEL_X64_H
#define EVENT_INTEL_X64_H
...
enum trigger_mode { edge, level };
enum level { deassert, assert, dontcare };

using vector_type = uint64_t;
using vcpuid_type = uint64_t;
using trigger_type = enum trigger_mode;
using level_type = enum level;

struct irq_desc {
    vector_type vector;
    vcpuid_type dest;
    trigger_type trig_mode;
    level_type level;
};

using irq_desc_type = struct irq_desc;
