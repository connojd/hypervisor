...
vmcs_intel_x64::launch(host_state_t host_state, guest_state_t guest_state)
{
    ...
    this->write_16bit_control_state();
    this->write_64bit_control_state();
    this->write_32bit_control_state();
    this->write_natural_control_state();
    ...
    m_pre_launch_delegate(host_state, guest_state);
    ...
    // NOTE:
    //
    // The post delegate only runs when is_guest() == false. If is_guest
    // is true, the guest launches, and this code is never executed.
    //

    m_post_launch_delegate(host_state, guest_state);
}
